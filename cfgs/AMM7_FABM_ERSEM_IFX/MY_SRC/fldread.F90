...
CONTAINS
   ...
   END SUBROUTINE fld_map
	...
	SUBROUTINE fld_map_core( pdta_read, kmap, pdta_bdy )
	  ...
	  !--------------------------------------------------------------
	  ! safety check: exit if kmap is not allocated or empty
	  !--------------------------------------------------------------
	  kmap_size = SIZE(kmap)
	  IF (kmap_size <= 0) RETURN
	  ...
	  ELSE
		 DO jk = 1, idim_bdy(3)
			DO jb = 1, idim_bdy(1)
			   IF (kmap(jb) > 0 .AND. kmap(jb) <= SIZE(pdta_read,1)) THEN
				  pdta_bdy(jb,1,jk) = pdta_read(kmap(jb),1,jk)
			   ELSE
				  pdta_bdy(jb,1,jk) = 0.0_wp   ! safe default for invalid indices
				  WRITE(*,*) 'Rank=', mpprank, ' jb=', jb, ' idim_bdy(1)=', idim_bdy(1), &
           ' SIZE(pdta_read)=', SIZE(pdta_read,1), SIZE(pdta_read,2), SIZE(pdta_read,3)
			   END IF
			END DO
		 END DO
	  END IF
